...
class DataInsight():
	...
	def __init__(self):
		...
		encode_target = ['hotel','agent','arrival_date_day_of_month','assigned_room_type','company','country','customer_type','deposit_type','distribution_channel','market_segment','meal','reserved_room_type']
		data_not_encoded = pd.concat([self.dataset.get_feature(encode_target),self.dataset.get_train_is_canceled()],axis = 1)
		...
		self.dataset.add_feature(data_encoded)
		self.dataset.remove_feature(encode_target)
		self.train_df = self.dataset.train_test_df[self.dataset.train_test_df['dataset'].eq(0)]
		self.train_df = pd.concat([self.train_df,self.dataset.get_train_is_canceled()],axis = 1)
		print(self.train_df)
	...
	def plot_attribute_is_canceled_mean(self, attribute):
		...
		for i in attribute_element:
			...
			std = np.std(self.train_df.loc[self.train_df[attribute] == i].loc[:, 'is_canceled'].to_numpy())
